...
if command -v emcc >/dev/null && command -v nim >/dev/null; then
  ...
  (cd "${repo_dir}" && nim c --hints:off -d:emscripten \
    replay-viewer/commons_family_replay.nim)
else
  # Fall back to the pinned emsdk container.
  image_tag="commons-family-replay-viewer-build:$$"
  docker build --platform linux/amd64 \
    --file "${repo_dir}/Dockerfile.replay-viewer" \
    --tag "${image_tag}" "${repo_dir}"
  container_id="$(docker create "${image_tag}")"
  rm -rf "${repo_dir}/replay-viewer/dist"
  docker cp "${container_id}:/workspace/commons-family/replay-viewer/dist" \
...
test -s "${dist}/commons_family_replay.wasm"
test -s "${dist}/commons_family_replay.js"

# `coworld build` pre-creates the parent of the output dir; CI does not, so
# make it here before anything writes into it.
mkdir -p "$(dirname "${output_dir}")"
rm -rf "${output_dir}"
mkdir -p "${output_dir}/assets"
cp "${dist}/commons_family_replay.js" "${dist}/commons_family_replay.wasm" \
  "${output_dir}/"
cp "${repo_dir}/replay-viewer/index.html" \
  ...
  "${output_dir}/"
for asset in cog_red.png cog_blue.png cog_green.png cog_yellow.png \
  cog_violet.png cog_orange.png apple.png mushroom_red.png \
  mushroom_green.png mushroom_blue.png arena_floor.png font.ttf; do
  cp "${repo_dir}/data/${asset}" "${output_dir}/assets/"
done

test -f "${output_dir}/index.html"
grep -q 'data-replay' "${output_dir}/static_replay.js"
echo "commons-family replay viewer bundle: ${output_dir}"
